...
export default class AuthController {
    ...
    public async signin(req: express.Request, res: express.Response){
        try{
            ...
            res.status(200).json({auth_token: this.tokenService.encode(id)})).catch(
                (err: Error) => res.status(404).json({error:err.message}))
        }catch(err){
    ...
    public async signout(req: express.Request, res: express.Response){
        try{
            ...
            res.status(200).json({message: result})).catch(
                (err: Error) => res.status(404).json({error:err.message}))
        }catch(err){
            return res.status(400).json({error: err})
